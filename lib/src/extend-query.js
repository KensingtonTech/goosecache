...
module.exports = function(mongoose, gooseCache, logger) {
  ...
  mongoose.Query.prototype.exec = async function(op, callback = noop ) {
    ...
    return await new Promise(async (resolve, reject) => {
      ...
      const onCachedResultsFound = async (cachedResults) => {
        ...
        try {
          ...
          log.debug('mongoose.Query.prototype.exec(): Setting result in cache');
          if (this._derivedKey) {
      ...
      if (this._cacheGetScript) {
        ...
        const res = await gooseCache.evalSha(...args);
        onCachedResultsFound(res)
      }
      else {
        log.debug('mongoose.Query.prototype.exec(): Getting results from cache with cache.get(), key:', key);
        const res = await gooseCache.get(key);
        onCachedResultsFound(res);
      }
    });
  };
